...
- 真源：Outbox Thread（协作总线线程）+ PR/commit（代码真源）+ `<outbox_repo>/.agents/workflow.toml`（配置真源）
...
- Outbox Thread：协作总线线程。V1 的默认承载由 `<outbox_repo>/.agents/workflow.toml` 的 `[outbox]` 段决定：可以是 GitHub/GitLab Issue，也可以是本地 SQLite thread（见 `docs/operating-model/outbox-backends.md`）。
