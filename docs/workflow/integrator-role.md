...
- `docs/workflow/lead-worker.md`：Lead/Worker 边界（推荐：`integrator-lead` 单写者）
...
> 本文件中的 “Integrator 做 X” 默认指 “Integrator-Lead 负责协调与写回”，而不是让 integrator 自己去写所有实现代码。
...
- 若需要明确“谁在收敛”：将 Issue 的 `assignee` 设置为 `integrator-lead`（或项目约定的 integrator 身份）。
...
- 推荐规则：`state:*` 的推进只允许一个角色写（推荐 `integrator-lead` 单写者）。见 `docs/workflow/guardrails.md`。
...
- 必须由 `integrator-lead` 汇总为唯一的 `FixList vN`，作为下一轮执行的唯一指令，并明确 `Next:` 指派。
...
- 如果需要你来收敛：将 assignee 指向 `integrator-lead`，并声明 `ReadUpTo/Trigger` 防止重复处理。
