...
- Claim 的真源 = Outbox backend 的 `assignee` 字段（被 assign 才算领取）
  ...
  - SQLite：`outbox_threads.assignee`
...
Issue（Outbox Thread）负责协作真源与状态机；PR 负责代码真源与 review/merge。
...
- Role Lead（backend lead / frontend lead / architect lead）：review PR（approve 或 request changes），并把关键结论回填到 Outbox Thread。
...
- `IssueRef: <owner/repo#number>`（必须；指向 Outbox issue）
...
- V1.1 的关闭真源仍然是 Outbox Thread 的状态机与 Approver 的 close 动作。

同时在 Outbox Thread 用 comment 模板回填：
...
当 Lead 在 PR 里 request changes 时，必须同步回填到 Outbox Thread（避免信息只留在 PR）：
...
- 无论单仓库还是多仓库：协作真源都仍是同一个 Outbox Thread（Issue 线程）。

## 5) Outbox Thread 抽象（为 GitLab/MySQL 预留）
